...
__randomPassPhrase ()__

Returns a random hex string that can be used as a pass phrase for encrypt(). The string is a bnary hexadecimal string of length 256 being generated from 128 randomly generted bytes. 

Because of Nodes crypto functions a new pass phase should be used for every encrypt operation. This pass phrase should be saved so that the plain text can be regenerated usng decrypt(). If the pass phrase is lost there is no way of regenerating the oringinal text.
